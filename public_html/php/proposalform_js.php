<script>
    ...
    var err_array = {
            ...
            'available_pos' : 'Il numero di posizioni disponibili deve essere compreso tra 1 e 500.',
            ...
            'mysql' : 'Caricamento informazioni proposta non riuscito, attendere qualche istante e riprovare.'
    };
    ...
    /** ----- load data from db ----- */
    function loadDbData( jQuery ) {
        <?php
            if(!empty($row)) {
                echo 'document.getElementById("'.$name.'").value="'.$row['name'].'";';
                echo 'document.getElementById("'.$description.'").value="'.str_replace("<br />","",$row['description']).'";';
                echo 'document.getElementById("'.$available_pos.'").value="'.$row['available_positions'].'";';
    ...
    <?php
        if ($error_flag) // se errore allora comunica all'utente ciò quando la pagina è ricaricata (funzione jquery)
            ...
            echo '$(document).ready(loadDbData);';
    ?>
</script>